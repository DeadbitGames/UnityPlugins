...
public class DebugConsole : MonoBehaviour
{
	void Start()
	{
		scrollRect = new Rect(10,20,_windowRect.width-20,_windowRect.height-60);
		inputRect = new Rect(10, scrollRect.yMax+10, _windowRect.width - 220, 24);
		enterRect = new Rect(inputRect.xMax+10, inputRect.y, 50, 24);
		toolbarRect = new Rect(enterRect.x+enterRect.width+5, enterRect.y, 135, 25);
		messageLine = new Rect(10,0,scrollRect.width-25,20);
	}
	...
	{
		get { return Instance._isOpen; }
		set { Instance._isOpen = value; }
	}
	...
	protected void Awake()
	{
		if (Instance != null)
		{
			Debug.LogError("There are multiple singletons on the scene!", Instance);
		}
		Instance = this;
	...
	void OnEnable()
	{
		...
		#else
		_windowRect = new Rect(30.0f, 30.0f, Screen.width-60, Screen.height-60);
		#endif
	...
	void OnGUI()
	{
		...
		#if !MOBILE
		_windowRect = GUI.Window(-1111, _windowRect, windowMethods[toolbarIndex], string.Format("Debug Console \tfps: {0:00.0}", fps.current));
		GUI.BringWindowToFront(-1111);
		GUI.FocusWindow(-1111);
		...
		#endif
		GUI.Box(_fakeWindowRect, string.Format("Debug Console v{0}\tfps: {0:00.0}", fps.current), dragging ? windowOnStyle : windowStyle);
	...
	{
		Instance.LogMessage(Message.Log(message));
	...
	{
		Instance.LogMessage(new Message(message, messageType));
	...
	{
		Instance.LogMessage(new Message(message, displayColor));
	...
	{
		Instance.LogMessage(new Message(message, messageType, displayColor));
	...
	{
		Instance.LogMessage(Message.Warning(message));
	...
	{
		Instance.LogMessage(Message.Error(message));
	...
	{
		Instance.EvalInputString(commandString);
	}
	...
	{
		Instance.RegisterCommandCallback(commandString, commandCallback);
	}
	...
	{
		Instance.AddWatchVarToTable(watchVar);
	}
	...
	{
		Instance.RemoveWatchVarFromTable(name);
	}
	#endregion
...
namespace UnityMock
{
	public static class Debug
	{
		...
		{
			UnityEngine.Debug.Log(message  + "dupa");
		}
